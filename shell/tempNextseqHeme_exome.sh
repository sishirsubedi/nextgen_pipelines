...
for file in /home/nextSeq_heme/$runName/*.vcf
	...
	if [[ $file =~ .comb.vcf ]]
		...
		if [ ! -z $ampliconRef ]
			...
			ln -s $file /home/"$instrument"Analysis/$runName/$sampleName.amplicon.vcf         #create symbolic link
		fi
		...
		/opt/samtools-1.4/samtools-1.4/samtools bedcov  /doc/ref/Heme/truseq-exome-targeted-regions-manifest-v1-2_run0080.bed /home/nextSeq_heme/$runName/$sampleName.sort.bam | awk ' {print $4,"\t",int($13/($8-$7))} ' > /home/"$instrument"Analysis/$runName/$sampleName.samtools.coverageDepth
		ampliconFile=$(ls /home/"$instrument"Analysis/$runName/$sampleName.samtools.coverageDepth)
		###filter amplicon file
		if [ ! -z $excluded ]
		then
		grep -v -f $excluded $ampliconFile > /home/"$instrument"Analysis/$runName/$sampleName.amplicon.filtered.txt
		else
		ln -s $ampliconFile /home/"$instrument"Analysis/$runName/$sampleName.amplicon.filtered.txt
		fi
	
	
	
		##split amplicon file
		sampleCol=$(head -n 1 /home/"$instrument"Analysis/$runName/$sampleName.amplicon.filtered.txt | awk -F"\t" '{print NF; exit}')
		...
		cut -f 1,$sampleCol /home/"$instrument"Analysis/$runName/$sampleName.amplicon.filtered.txt > /home/"$instrument"Analysis/$runName/$sampleName.amplicon.txt
