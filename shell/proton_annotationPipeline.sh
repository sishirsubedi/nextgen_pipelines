...
split_protonVCF()
{
	/storage/apps/opt/python3/bin/python3  ${HOME_PYTHON}splitVcf.py \
...
run_vep()
{
  ...
  if [ ! -f "${HOME}${CALLERID}/TSVC_variants.filter.split.vep.vcf" ] ; then
    ...
    update_status "$QUEUEID" "ERROR:VEP" "$DB" "$USER"  "$PASSWORD" "$DB_HOST"
    exit
...
parse_vep()
{
  log_info "Parsing VEP results"
	...
	/storage/apps/opt/python3/bin/python3 ${HOME_PYTHON}parseVEP_v2.py \
					-m parseIonNewVarView \
...
filter_vep()
{
  ...
  /storage/apps/opt/python3/bin/python3  ${HOME_PYTHON}filterVEP.py \
