...
def checkRef(df_snp_mut_2,detail):
    ...
    df_join = pd.merge(left=df_snp_mut_1, right=df_snp_mut_2, on=['CHROM','POS','REF','ALT'],how='right',indicator=True)
    df_join.to_csv(OUT_DIR+"temp.txt",sep='\t')

    if detail:
        plt.xlim(5,20)
        sns.distplot(df_join[( (df_join['_merge']=='both') & (df_join['SomaticEVS'] > 5 ) )]['SomaticEVS'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[(  (df_join['_merge']=='right_only') & (df_join['SomaticEVS'] > 5 ))]['SomaticEVS'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'SomaticEVS_2')
        plt.close()

        plt.xlim(0,200)
        sns.distplot(df_join[( (df_join['_merge']=='both') & (df_join['NORMAL.DP'] <200 ) )]['NORMAL.DP'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[(  (df_join['_merge']=='right_only') & (df_join['NORMAL.DP'] <200 ))]['NORMAL.DP'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'NORMAL_DP_2')
        plt.close()

        plt.xlim(0,200)
        sns.distplot(df_join[( (df_join['_merge']=='both') & (df_join['TUMOR.DP'] <200 ) )]['TUMOR.DP'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[(  (df_join['_merge']=='right_only') & (df_join['TUMOR.DP'] <200 ))]['TUMOR.DP'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'TUMOR_DP_2')
        plt.close()

        plt.xlim(0,200)
        sns.distplot(df_join[( (df_join['_merge']=='both') & (df_join['QSS'] <200 ) )]['QSS'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[(  (df_join['_merge']=='right_only') & (df_join['QSS'] <200 ))]['QSS'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'QSS_2')
        plt.close()


        plt.xlim(0,200)
        sns.distplot(df_join[( (df_join['_merge']=='both') & (df_join['QSS_NT'] <200 ) )]['QSS_NT'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[(  (df_join['_merge']=='right_only') & (df_join['QSS_NT'] <200 ))]['QSS_NT'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'QSS_NT_2')
        plt.close()


        sns.distplot(df_join[df_join['_merge']=='both']['SomaticEVS'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[df_join['_merge']=='right_only']['SomaticEVS'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'SomaticEVS')
        plt.close()


        sns.distplot(df_join[df_join['_merge']=='both']['NORMAL.DP'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[df_join['_merge']=='right_only']['NORMAL.DP'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'NORMAL_DP')
        plt.close()


        sns.distplot(df_join[df_join['_merge']=='both']['TUMOR.DP'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[df_join['_merge']=='right_only']['TUMOR.DP'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'TUMOR_DP')
        plt.close()

        sns.distplot(df_join[df_join['_merge']=='both']['QSS'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[df_join['_merge']=='right_only']['QSS'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'QSS')
        plt.close()


        sns.distplot(df_join[df_join['_merge']=='both']['QSS_NT'], color='green',kde=False,label='TP',bins=50)
        sns.distplot(df_join[df_join['_merge']=='right_only']['QSS_NT'], color='red',kde=False,label='FP',bins=50)
        plt.legend()
        plt.savefig(OUT_DIR+'QSS_NT')
        plt.close()



    return([df_join.shape,df_join[df_join['_merge']=='right_only'].shape[0],df_join[df_join['_merge']=='both'].shape[0]])
...
print('SNP After GL/gl:'+ str(df_snp.shape[0]))
df_snp = df_snp[ df_snp['FILTER']=="PASS"]
print('SNP pass only:', df_snp.shape)
...
print('INDEL After GL/gl:'+ str(df_indel.shape[0]))

# df_indel = df_indel[ df_indel['FILTER']=="PASS"]
# print('INDEL pass only:', df_indel.shape)
#
